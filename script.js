...
const firebaseConfig = {
  apiKey: "AIzaSyDkN2DXzvcQ9b0qVG2JUxyv_yXKIVxYYTI",
  authDomain: "lipetz-d8ab3.firebaseapp.com",
  projectId: "lipetz-d8ab3",
  storageBucket: "lipetz-d8ab3.firebasestorage.app",
  messagingSenderId: "789434931399",
  appId: "1:789434931399:web:d0495581e5b858b382c6e2",
  measurementId: "G-WH3EGC2HBL"
};
};
// --- 🔼 PASTE YOUR FIREBASE CONFIG OBJECT HERE 🔼 ---
...
import { 
    getFirestore, doc, getDoc, setDoc, collection, addDoc, onSnapshot, serverTimestamp, query, orderBy
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        document.getElementById('loading-overlay').innerHTML = `<p class="text-red-500">שגיאה בטעינת האתר. אנא ודא שהדבקת את פרטי ה-Firebase הנכונים בקובץ script.js.</p>`;
    }
});
...
async function loadTripData() {
    ...
    if (docSnap.exists()) {
        ...
        document.getElementById('loading-overlay').innerHTML = '<p class="text-red-500">לא נמצא מידע על הטיול בבסיס הנתונים. האם השתמשת בכלי ההעלאה?</p>';
...
function setupRealtimeListeners() {
    const messagesQuery = query(collection(db, "trips/geneva2025/messages"), orderBy("timestamp", "asc"));
    ...
    const expensesQuery = query(collection(db, "trips/geneva2025/expenses"), orderBy("timestamp", "desc"));
...
function renderActivities() {
    ...
    categories.forEach(cat => {
        if(cat && cat !== 'בית מרקחת') {
             filterContainer.innerHTML += `<button class="btn-filter px-4 py-2 rounded-full" data-filter="${cat}">${cat}</button>`;
        }
    });

    const currentFilter = document.querySelector('#activities .btn-filter.active').dataset.filter;
...
function renderMessages(messages) {
    ...
    if(!messages || messages.length === 0) {
        container.innerHTML = '<p class="text-center text-gray-500">עדיין אין הודעות. היו הראשונים לכתוב!</p>';
        return;
    };
...
function renderExpenses(expenses) {
    ...
    if (!expenses || expenses.length === 0) {
        list.innerHTML = '<p class="text-center text-gray-500">אין הוצאות רשומות.</p>';
        totalEl.innerHTML = 'סך הכל (מוערך): ₪0.00';
        return;
    };
    ...
    expenses.forEach(exp => {
        ...
        const rate = CURRENCY_RATES[exp.currency]?.ILS || 1;
        totalILS += exp.amount * rate;
    });
...
function renderPackingGuide() {
    ...
    container.innerHTML = tripData.packingGuide.htmlContent; 
}
...
function handleWhatsAppShare(event) {
    ...
    const dayIndex = parseInt(dayElement.dataset.dayIndex, 10);
...
function setupEventListeners() {
    ...
    document.querySelectorAll('[id^="open-"]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const modalId = e.currentTarget.id.replace('open-', '').replace('-btn', '').replace('-mobile', '') + '-modal';
            const modal = document.getElementById(modalId);
            if(modal) {
                modal.classList.remove('hidden');
                modal.classList.add('flex');
                if (modal.id === 'family-details-modal') populateFamilyDetails();
            }
        });
    });
    ...
    // Placeholder for Gemini Chat functionality
    document.querySelectorAll('.nav-gemini-btn').forEach(btn => btn.addEventListener('click', () => alert('Gemini Chat feature coming soon!')));
...
function updateProgressBar() {
    if (!tripData.tripTimeline) return;
...
function displayCurrentActions() {
    if(!tripData.timeSensitiveActions || !tripData.tripDates) return;
    const now = new Date();
...
function populateFamilyDetails() {
    ...
    if (!container || !tripData.family) return;
    let html = '';
    tripData.family.forEach(member => {
        html += `<div class="flex justify-between p-2 border-b"><span class="font-semibold">${member.name}:</span><span>${member.passport}</span></div>`;
    });
